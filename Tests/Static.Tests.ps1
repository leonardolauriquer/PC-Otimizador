...
Assert-Contains $engine 'function Get-RecycleBinSizeBytes' 'Recycle Bin size is measured'
Assert-Contains $engine 'function Test-NonFatalBatchAction' 'Optional restore/ARP/NetBIOS do not fail the batch'
Assert-Contains $engine 'param([switch]$Force)' 'Cancel flag can preserve a recent GUI STOP'
Assert-Contains $gui 'void UnscheduleWeekly()' 'GUI can remove weekly schedule'
Assert-Contains $gui '-WindowStyle Minimized' 'GUI engine PowerShell is minimized not hidden'
Assert-Contains $gui 'CreateNoWindow = false' 'GUI engine PowerShell allocates a window'
Assert-NotContains $gui '_english ? T("status.activity") : msg' 'English status bar shows the real log line'
Assert-Contains $cli '8. Dry-run Limpeza Segura' 'CLI option 8 matches Executar.bat dry-run'
